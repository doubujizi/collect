package com.dou.tfx.prefect.utils;
...
public class RedisUtils {
     ...
     * @author Bei·S·J
     */
    public boolean isActionAllowed(String comId, String account, String taskType, int period, int maxCount) {

        final long current = System.currentTimeMillis();
        // 移除时间窗口之前的行为记录，剩下的都是时间窗口内的
        String key = "auto:account:limit:" + comId + "-" + account + ":" + taskType;
        this.redisTemplate.opsForZSet().removeRangeByScore(key, 0, current - period * 1000);
        // 获取窗口内的行为数量
        Long zCard = redisTemplate.opsForZSet().zCard(key);
        if (zCard < maxCount) {
            // 记录行为 score
            this.redisTemplate.opsForZSet().add(key, String.valueOf(current), current);
            // 设置zset过期时间，避免冷用户持续占用内存
            // 过期时间应该等于时间窗口长度，再多宽限1s
            this.redisTemplate.expire(key, period + 1, TimeUnit.SECONDS);
            return true;
        }
        return false;
    }
